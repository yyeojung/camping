...
export default function CampingCard({ className }: IPropsList) {
    ...
    async function fetchData(): Promise<void> {
      ...
      try {
        const response = await axios.get<IApiResponse>(
          `http://apis.data.go.kr/B551011/GoCamping/basedList?serviceKey=${SERVICE_KEY}&numOfRows=${PER_PAGE}&pageNo=${pageCount}&MobileOS=AND&MobileApp=appName&_type=json`,
        );
  ...
  const pageCount = Math.ceil(40 / PER_PAGE); // 전체 페이지 수
